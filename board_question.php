...
if(!empty($_POST)){
  ...
  if(empty($err_msg)){
    ...
    try {
      ...
      $data = array(':message' => $msg, ':user_id' => $_SESSION['user_id'], ':board_id' => 2, ':created_date' => date('Y-m-d H:i:s'));
...
$dbh = dbConnect();
$posts = $dbh->query('SELECT u.username, u.pic, m.* FROM users u, message m WHERE u.id = m.user_id AND board_id = 2 ORDER BY m.created_date DESC');
